...
assert(
  snapshot.draftCards.every(
    (card) =>
      draftCardHasRequiredFields(card) && draftCardKeepsFullReportPrivate(card),
  ),
...
assert(
  getPulseFreshness(snapshot.weeklyPulse, new Date("2026-09-06T18:00:00.000Z")) ===
    "stale",
  "a pulse older than its configured window must be stale",
);
assert(
  getPulseFreshness(snapshot.weeklyPulse, new Date("2026-09-04T18:30:00.000Z")) ===
    "fresh",
