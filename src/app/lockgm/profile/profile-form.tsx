...
export function LockgmProfileForm({
  ...
  const [gmIdHandle, setGmIdHandle] = useState(
    profile.gmId.replace(/^GM-/, ""),
  );
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      setGmIdHandle(result.profile.gmId.replace(/^GM-/, ""));
      setMessage(`Profile saved. Your GM ID is ${result.profile.gmId}.`);
      router.refresh();
    });
  }
      ...
      <section className="border border-[color:var(--lg-line)] bg-[color:var(--lg-panel)] p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="border border-[color:var(--lg-accent)]/40 px-3 py-2 text-right">
            <label
              htmlFor="gmIdHandle"
              className="block text-[10px] font-bold tracking-[0.16em] text-[color:var(--lg-mute)] uppercase"
            >
              Stable GM ID
            </label>
            <div className="mt-1 flex items-center justify-end gap-1">
              <span className="lockgm-display text-xl font-bold text-[color:var(--lg-accent)]">
                GM-
              </span>
              <input
                id="gmIdHandle"
                ...
                minLength={3}
                maxLength={20}
                className="lockgm-display w-32 rounded-md border border-[color:var(--lg-line)] bg-[color:var(--lg-bg)] px-2 py-1 text-right text-xl font-bold text-[color:var(--lg-accent)] outline-none focus:border-[color:var(--lg-accent)]"
                placeholder="SHADOW42"
              />
            </div>
          </div>
        </div>
        <p className="mt-4 max-w-2xl text-sm leading-relaxed text-[color:var(--lg-mute)]">
          ...
          the old ID. It is not a government identifier.
        </p>
