...
export const SENTI_THINKS_DELIVERY_RULE =
  "HARD RULE: a delivery-platform Seed (Hometown Runner, Home Town Runnner) is DoorDash-style — three role-based logins (customer, merchant, driver), restaurant portal, driver/scout portal, admin ledger. Senti must think through those apps. Platform food share stays $0 on restaurant orders (subscriptions only). Hometown charges the restaurant $0.25 per order for API expenses — not the diner, driver, or scout. The 10% is 5% scout + 5% driver. Stripe three-party Connect: restaurant, scout, and driver each have a Stripe account. Subscriptions and payouts use Stripe Connect ACH, not card. Menu is photo/PDF first (snap the paper takeout menu, five-minute sign-off, 86 during service) with website crawl as fallback — no Toast / Square / Otter / POS certification. Trip is $4.50 + $1.50/mile. Driver software ($39/$79) after 60 days free from the first successful drive. 60-day suspend. Onboarding: license, insurance, existingPlatformActive (DoorDash / Uber Eats) — no expensive background check. One-page IC agreement (liability, vehicle, indemnification). Core dispatch only: FIXED 2-mile merchant radius, 300 meter kitchen arrival geofence (Driver Arrived), 100 feet drop-off check. Scout moat: Riley sends dine-in customers (e.g. seven couples/week) because FSR traffic is ~70% dine-in and only ~5% delivery. Scout pay: one delivery a month or the 5% cascades to the next most-active signed scout at that kitchen. A restaurateur can scout another kitchen after one delivery — they cannot keep the 5% on their own. Do not copy a dining-room, bakery, or pizza restaurant format. Diner landing stays guest-only — no 1099 / $39 / GMV / Stripe / geofence / IC / ACH / background check / OCR / 86 / Red Card / POS / API expense / $0.25.";
...
function deliveryChapters(input: {
  ...
  return [
    ...
    },
    {
      ...
      script: `One job only for ${site}: the marketplace. IN SCOPE: customer order nearby, /portal restaurant desk (pause/online, accept/decline tickets, Driver Arrived staging), /portal driver desk (go online, accept run, 300m kitchen arrival, 100ft drop-off), admin ledger, FIXED 2-mile radius. OUT OF SCOPE: reserve a table, pizza with personality, chef’s dinner plates, kitchen-ticket single-room copy, a $2,000/week restaurant promise, dynamic 8–10 mile expansion, Premier visibility tiers, weather/supply algorithms, KDS fry-fire. Source of truth is the product brief — not a restaurant website template.`,
      status: "proposed",
    },
    {
      id: "pixel",
    ...
    },
    {
      ...
      script: `Hard rules for ${site}: platform food share stays $0 from restaurant orders (revenue is driver subscriptions billed on Stripe Connect ACH, not card); Hometown charges the restaurant $0.25 per order for API expenses — not the diner, driver, or scout; restaurants pay 10% on delivery GMV fully split 5% scout + 5% driver; drivers keep 100% of fee + tip + the 5% share via Stripe Connect (direct to the driver); restaurant collects the food total and pays ~2.9% processor plus the $0.25 API expense; trip is $4.50 + $1.50/mile so drivers clear the $0.76 federal mileage rate; payouts fire at a $25 minimum balance or weekly via ACH; drivers manage their own tax forms (restaurant does not issue 1099s); everybody gets 60 days free starting the first successful drive (a delivered run — not signup, first login, or first offer); after day 60 driver software is $39/month part-time or $79/month full-time (weekly $9.99 / $19.99); full-time is app-open more than 30 hours/week or 120 hours in 4 weeks; 60 days without opening the app auto-suspends the driver; onboarding is license + insurance + existingPlatformActive — no expensive background check; one-page IC agreement; FIXED 2-mile radius, 300 meter kitchen geofence, 100 feet drop-off. Fail closed if money is unclear. Do not invent “Reserve a table”, “Pizza With Personality”, “how guests use the room”, or kitchen-ticket chrome.`,
      status: "proposed",
    },
    {
      id: "lumen",
      ...
      script: `Admin is the ops ledger — not a host stand. Accounting is 10% fully split 5% scout / 5% driver, $0 platform food share on the order, plus $0.25 API expense from the restaurant. CRM/ops is the restaurant roster plus scout attribution. Market: DoorDash has no published US AOV (Rakuten $37.28; Q4 2025 implied ~$33 global); 8M+ US / 9M+ world Dashers in 2025, typical ~10 weeks / 4 hours. FSR mix 70% dine-in / 5% delivery. Independent median ~$850K (rough). Riley sends dine-in first (seven couples in a week), then those tables become delivery through Riley. Delivery is the product: restaurant portal + driver portal, DoorDash concept. Do not mark delivery N/A. Do not spec a single dining room.`,
      status: "proposed",
    },
    {
      id: "sentry",
...
export function exampleSeedPlaybook(): SeedPlaybook {
  return compileSeedPlaybook({
    ...
    brief:
      "Hometown Runner is a hyper-local food delivery platform. Three role-based logins. Platform food share stays $0 on restaurant orders. Hometown charges the restaurant $0.25 per order for API expenses. The 10% is 5% scout + 5% driver. Stripe three-party Connect: restaurant, scout, and driver each have a Stripe account. Subscriptions and payouts use Stripe Connect ACH, not card. Menu is photo/PDF first — snap the paper takeout menu, five-minute sign-off, 86 during service; website crawl is fallback. No POS certification. Trip is $4.50 + $1.50/mile. Everybody gets 60 days free from the first successful drive; after that driver software is $39/month part-time or $79/month full-time. Drivers manage their own tax forms. Onboarding is license, insurance, and existingPlatformActive — no expensive background check. One-page IC agreement. Core dispatch: FIXED 2-mile merchant radius, 300 meter kitchen arrival geofence, 100 feet drop-off. Riley sends dine-in customers (seven couples in a week) because full-service traffic is about 70% dine-in and only 5% delivery. Scout pay: one delivery a month or the 5% cascades to the next most-active signed scout at that kitchen. A restaurateur can scout another kitchen after one delivery — they cannot keep the 5% on their own.",
    seedMode: "build",
    liveUrl: null,
  });
}
