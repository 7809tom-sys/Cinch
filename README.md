...
**Brand:** user-facing name is **LockedGM**. Routes, folders, env keys, and package scripts keep the stable `lockgm` / `LOCKGM_*` technical identifiers — see `docs/lockgm-brand.md`.
...
- `/lockgm/scouting` — HS Top 100, MiLB Top 200, college football Top 300, 2027 NFL free-agent reports + premium write-ups
- `/lockgm/pricing` — Shadow / War Room / Pipeline tiers

### Give LockedGM its own domain

LockedGM can also be served at the root of a domain you already own (e.g.
...
1. **Admin → Domains → "LockedGM's own domain"**: connect the domain, add the
